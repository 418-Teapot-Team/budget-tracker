<template>
  <div
    class="w-full h-44 shadow-sm bg-white rounded-2xl flex flex-row justify-between items-center p-6 hover:bg-gray-50"
  >
    <div class="h-full w-28 mr-10">
      <deposit-card-icon />
    </div>
    <div class="w-full h-full flex flex-col justify-between items-center mr-5">
      <div class="w-full h-1/2 flex flex-row justify-between items-start">
        <span class="text-3xl"> Deposit title </span>
        <div class="flex flex-col justify-start items-end">
          <span class="text-black">100 / 10000 UAH</span>
          <span class="text-black">Ставка 5% ріних</span>
        </div>
      </div>
      <div class="w-full">
        <div class="w-full h-3 bg-black rounded-full bg-opacity-10 mb-3">
          <div class="h-3 bg-black rounded-full" style="width: 30%"></div>
        </div>
        <div class="w-full flex flex-row justify-between items-start">
          <span class="text-black"> 10.05.2022 </span>
          <span class="text-black"> 17.09.2023 </span>
        </div>
      </div>
    </div>
    <div class="w-7 h-full flex flex-col justify-start items-end gap-3">
      <div class="w-7 h-7 cursor-pointer" @click="deleteDeposit">
        <delete-icon />
      </div>
      <div class="w-7 h-7 cursor-pointer" @click="editDeposit">
        <edit-icon />
      </div>
    </div>
  </div>
</template>

<script>
import DepositCardIcon from '@/components/Icons/DepositCardIcon.vue';
import DeleteIcon from '@/components/Icons/DeleteIcon.vue';
import EditIcon from '@/components/Icons/EditIcon.vue';
export default {
  name: 'DepositCard',
  components: {
    DepositCardIcon,
    DeleteIcon,
    EditIcon,
  },
  methods: {
    editDeposit() {
      this.$emit('onEditDeposit', 'id');
    },
    deleteDeposit() {
      this.$emit('onDeleteDeposit', 'id');
    },
  },
};
</script>
